...
文字列は右書きのキャラクタを含まないが、BiDiラベルではない
(RTLラベルでもLTRラベルでもない)。

=item C<BIDIRULE_AVOIDED>

文字列は書式文字を含むため、BiDi規則を満たさない。認められなかったキャラクタを表示すると問題が起きるかもしれない。

=item C<BIDIRULE_INVALID>

文字列はその他の理由でBiDi規則を満たさない。
...
BiDiラベルではない文字列 (RTLラベルでもLTRラベルでもない文字列)
が無効な文字列であるとはかぎらない。別の規則で検査すべきである。
